...
local Settings = {
    ...
    -- DANH SÁCH ĐỒNG ĐỘI: Thêm tên tài khoản hoặc tên hiển thị của bạn bè vào đây
    -- Viết hoa, viết thường hoặc tên có dấu đều được, cách nhau bằng dấu phẩy
    Teammates = {
        "quyetcho989",
        "quyetbanana",
...
-- Hàm kiểm tra xem người này có nằm trong danh sách đồng đội không
local function IsTeammate(targetPlayer)
    local pName = string.lower(targetPlayer.Name)
    local pDisplayName = string.lower(targetPlayer.DisplayName)
    
    for _, name in pairs(Settings.Teammates) do
        local checkName = string.lower(name)
        -- Nếu trùng Tên tài khoản hoặc Tên hiển thị (Display Name) thì coi như đồng đội
        if pName == checkName or pDisplayName == checkName then
            return true
        end
    end
    return false
...
local function GetTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            -- BỎ QUA NẾU LÀ ĐỒNG ĐỘI TRONG DANH SÁCH
